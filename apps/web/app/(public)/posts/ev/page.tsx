  ...
  const filtered = useMemo(() => {
    ...
    if (appliedFilters.range) {
      switch (appliedFilters.range) {
        case '<300':
          data = data.filter((p) => p.batteryCapacityKWh < 30); // Approximate range based on capacity
          break;
        case '300-600':
          data = data.filter((p) => p.batteryCapacityKWh >= 30 && p.batteryCapacityKWh <= 60);
          break;
        case '>600':
          data = data.filter((p) => p.batteryCapacityKWh > 60);
          break;
      }
    }

    if (appliedFilters.brand) {
      data = data.filter((p) => p.brandName.toLowerCase().includes(appliedFilters.brand.toLowerCase()));
    }
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-4xl font-bold tracking-tight bg-gradient-to-r from-green-500 to-blue-500 bg-clip-text text-transparent mb-2">
              Xe điện (EV)
            </h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              className="border border-gray-300 rounded-lg px-4 py-2 bg-white shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all"
              value={sort}
