...
import { getCarPostsWithQuery } from '@/lib/api/postApi';
...
export function CarModal({ open, onOpenChange }: CarModalProps) {
  ...
  const products = cars.filter((product) =>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>Thêm xe điện để so sánh</DialogTitle>
          {isAtMaxLimit && (
