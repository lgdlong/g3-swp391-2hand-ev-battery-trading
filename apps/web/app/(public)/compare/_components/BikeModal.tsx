...
import { getBikePostsWithQuery } from '@/lib/api/postApi';
...
export function BikeModal({ open, onOpenChange }: BikeModalProps) {
  ...
  const products = bikes.filter((product) =>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>Thêm xe máy để so sánh</DialogTitle>
          {isAtMaxLimit && (
